...
@TeleOp(name="SpeedTeleop", group="Linear Opmode")
...
public class ShooterTuner extends LinearOpMode {
    ...
    static DcMotorEx shooterjerryPID;
    static VoltageSensor voltagesensor;
    static MoveDirection Direction;
    static PIDFCoefficients shooterPIDCoeff;
    ...
    public void runOpMode() {
        ...
        //shooterjerryPID = hardwareMap.get(DcMotorEx.class, "ShooterJerry");
        ...
        boolean button_dright_already_pressed = false;
        boolean button_dleft_already_pressed = false;
        ...
        if (opModeIsActive()) {
            ...
            while (opModeIsActive()) {
                ...
                //shooterPIDCoeff = shooterjerryPID.getPIDFCoefficients(DcMotor.RunMode.RUN_USING_ENCODER);
                ...
                if (button_dright_already_pressed == false) {
                    if (gamepad2.dpad_right) {
                        power = power + 0.01;
                        shooterjerry.setPower(power);
                        button_dright_already_pressed = true;
                    }
                }
                else {
                    if (!gamepad2.dpad_right) {
                        button_dright_already_pressed = false;
                    }
                }

                if (button_dleft_already_pressed == false) {
                    if (gamepad2.dpad_left) {
                        power = power - 0.01;
                        shooterjerry.setPower(power);
                        //shooterjerryPID.setPIDFCoefficients(DcMotor.RunMode.RUN_USING_ENCODER, shooterPIDCoeff);
                        button_dleft_already_pressed = true;
                    }
                }
                else {
                    if (!gamepad2.dpad_left) {
                        button_dleft_already_pressed = false;
                    }
                }
                ...
                telemetry.addData("Calc Rate", checkInterval);
                telemetry.addData("Shooter Speed", speed);
                //telemetry.addData("Shooter P", shooterPIDCoeff.p);
                //telemetry.addData("Shooter I", shooterPIDCoeff.i);
                //telemetry.addData("Shooter D", shooterPIDCoeff.d);
                ...
                //telemetry.addData("Volatage", voltagesensor.getVoltage());
